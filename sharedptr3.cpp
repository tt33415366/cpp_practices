...
int main()
{
	...
	for (int i = 0; i < 100; ++i) {
		/* Since `shared_ptr` provides only operator * and -> , we have to use `get()`, which 
		 * yields the internal pointer wrapped by `shared_ptr` to provide the full pointer 
		 * semantics.
		 */
		smp.get()[i] = i * 42;
	}
